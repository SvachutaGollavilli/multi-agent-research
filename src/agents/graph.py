...
def build_graph():
    ...
    Research pipeline graph:
        planner → researcher → analyst → synthesizer → writer → END

    synthesizer sits between analyst and writer:
      - analyst  : extracts isolated claims from individual sources
      - synthesizer: cross-references claims, resolves conflicts, unified narrative
      - writer   : turns the narrative into a formatted report
    """
