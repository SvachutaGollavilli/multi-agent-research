...
def synthesizer_agent(state: ResearchState) -> dict:
    ...
    event_id, t0 = log_agent_start(run_id, "synthesizer", {
    ...
    if not key_claims:
        log_agent_end(event_id, run_id, "synthesizer", t0, error="No claims to synthesise")
        return {
            "synthesis":      "",
            "source_ranking": [],
    ...
    try:
        ...
        source_ranking = [
            {
                "source_idx":   idx,
                "title":        sources[idx - 1].get("title", "Untitled") if idx <= len(sources) else f"Source {idx}",
        ...
        log_agent_end(event_id, run_id, "synthesizer", t0,
