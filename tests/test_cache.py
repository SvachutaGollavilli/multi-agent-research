...
class TestCacheFetch:
    ...
    def test_disabled_cache_always_misses(self, tmp_db_path):
        ...
        with (
            patch("src.cache.research_cache._DB_PATH", tmp_db_path),
            patch(
                "src.cache.research_cache.get_cache_config",
                return_value={
                    "enabled": False,
                    "ttl_seconds": 86400,
...
class TestCacheStore:
    def test_store_then_fetch(self, patched_cache, tmp_db_path):
        ...
        data = [
            {"title": "Stored", "url": "https://stored.com", "content": "content here"}
        ]
    ...
    def test_store_upserts_on_same_query(self, patched_cache, tmp_db_path):
        ...
        store(
            "upsert query",
            [{"title": "v1", "url": "https://a.com", "content": "c"}],
            "t",
        )
        store(
            "upsert query",
            [{"title": "v2", "url": "https://b.com", "content": "c"}],
            "t",
        )
...
class TestCacheStats:
    ...
    def test_stats_counts_stale_separately(self, patched_cache, tmp_db_path):
        ...
        _write_cache_row(
            tmp_db_path,
            "fresh query",
            [{"url": "https://f.com", "title": "f", "content": "c"}],
        )
        _write_cache_row(
            tmp_db_path,
            "stale query",
            [{"url": "https://s.com", "title": "s", "content": "c"}],
            created_at=old,
        )
        s = stats()
        assert s["total_entries"] == 2
