...
def _make_structured_llm_response(parsed_obj, raw_content="ok",
...
def _mock_observability():
    ...
    return {
        ...
        "write_report":   patch("src.agents.graph.write_report",    return_value="/tmp/report.docx"),
        "log_agent_start": patch("src.observability.logger.log_agent_start",
                                 return_value=("mock-event-id", 0.0)),
        "log_agent_end":  patch("src.observability.logger.log_agent_end",   return_value=None),
        "log_cost":       patch("src.observability.logger.log_cost",         return_value=None),
    }


FAKE_SOURCES = [
    {"title": "FAISS Overview",  "url": "https://engineering.fb.com/faiss",
    ...
    {"title": "FAISS Wikipedia", "url": "https://en.wikipedia.org/wiki/FAISS",
    ...
    {"title": "FAISS Tutorial",  "url": "https://www.pinecone.io/learn/faiss/",
...
class TestFullPipelineMocked:
    ...
    def _run_pipeline(self, query: str = "What is FAISS?"):
        ...
        analyst_obj = AnalystOutput(
            claims=[
                ClaimOutput(claim="FAISS supports billion-scale search",
                            source_idx=1, confidence="high",
                            evidence="FB blog states billion-scale support"),
                ClaimOutput(claim="FAISS runs on GPU",
                            source_idx=2, confidence="high",
                            evidence="Wikipedia confirms GPU support"),
                ClaimOutput(claim="IVF index partitions vectors into Voronoi cells",
                            source_idx=3, confidence="medium",
                            evidence="Pinecone tutorial explains IVF"),
                ClaimOutput(claim="FAISS is maintained by Meta AI Research",
                            source_idx=2, confidence="high",
                            evidence="Wikipedia states Meta AI Research"),
                ClaimOutput(claim="HNSW provides approximate nearest neighbor search",
                            source_idx=2, confidence="medium",
        ...
        with patch("langchain_anthropic.ChatAnthropic") as MockLLM, \
             ...
             patch("src.observability.logger.log_agent_start",
                   return_value=("evt-id", 0.0)), \
    ...
    def test_pipeline_trace_has_all_agents(self):
        ...
        required     = {"planner", "researcher", "merge_research",
                        "quality_gate", "analyst", "synthesizer",
                        "writer", "reviewer"}
...
class TestReviewerRefinementLoop:
    ...
    def test_low_score_loops_back_to_writer(self):
        from src.agents.graph import _should_revise
        state = default_state("q")
        state["review"]         = {"score": 4, "passed": False,
                                   "issues": ["missing citations"], "suggestions": []}
    ...
    def test_writer_uses_revision_prompt_on_second_call(self):
        ...
        with patch("langchain_anthropic.ChatAnthropic") as MockLLM, \
             patch("src.guardrails.scrub_pii",
                   return_value=("revised draft v2 content " * 30, [])), \
             patch("src.observability.logger.log_agent_start",
                   return_value=("evt", 0.0)), \
             patch("src.observability.logger.log_agent_end",   return_value=None), \
             patch("src.observability.logger.log_cost",         return_value=None):
...
class TestBudgetEnforcement:
    ...
    def test_accumulator_per_agent_cost_breakdown(self):
        from src.observability.cost import RunCostAccumulator, calculate_cost
        acc = RunCostAccumulator(run_id="test")
        acc.add(calculate_cost("claude-haiku-4-5-20251001", 300, 200, "planner", "test"))
        acc.add(calculate_cost("claude-haiku-4-5-20251001", 500, 300, "analyst", "test"))
...
class TestStateSchemaE2E:
    def test_default_state_has_all_fields(self):
        ...
        required = [
            "run_id", "query", "sub_topics", "research_plan",
            "current_topic", "force_research", "sources",
            "search_queries_used", "quality_score", "quality_passed",
            "quality_retries", "key_claims", "conflicts", "synthesis",
            "source_ranking", "drafts", "current_draft", "revision_count",
            "review", "final_report", "pipeline_trace", "errors",
