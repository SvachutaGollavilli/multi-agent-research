...
class TestSearchWikipedia:
    ...
    def test_returns_list(self):
        page = self._make_wiki_page(
            "FAISS", "https://en.wikipedia.org/wiki/FAISS", "FAISS is a library " * 30
        )
        with (
            patch("src.tools.wikipedia.wikipedia.search", return_value=["FAISS"]),
    ...
    def test_content_is_truncated_to_1000_chars(self):
        ...
        page = self._make_wiki_page(
            "FAISS", "https://en.wikipedia.org/wiki/FAISS", long_summary
        )
        with (
            patch("src.tools.wikipedia.wikipedia.search", return_value=["FAISS"]),
            patch("src.tools.wikipedia.wikipedia.page", return_value=page),
    ...
    def test_disambiguation_tries_first_option(self):
        ...
        with (
            patch("src.tools.wikipedia.wikipedia.search", return_value=["Python"]),
            patch(
                "src.tools.wikipedia.wikipedia.page",
                side_effect=[
                    wiki_module.DisambiguationError(
    ...
    def test_page_error_is_skipped(self):
        ...
        with (
            patch(
                "src.tools.wikipedia.wikipedia.search", return_value=["NonexistentPage"]
            ),
            patch(
                "src.tools.wikipedia.wikipedia.page",
                side_effect=wiki_module.PageError("NonexistentPage"),
            ),
        ):
            from src.tools.wikipedia import search_wikipedia
...
class TestAsyncSearchAll:
    ...
    def test_both_tools_merges_results(self):
        ...
        with (
            patch(
                "src.tools.async_search.async_search_web",
                new=AsyncMock(return_value=tavily),
            ),
            patch(
                "src.tools.async_search.async_search_wikipedia",
